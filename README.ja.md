...
- **OCR テキスト抽出** — 画像ベースの固定レイアウト EPUB（雑誌・コミック等）
  向けに、**翻訳 > 画像ページからテキスト抽出 (OCR)…** で全ページの画像を
  Ollama のビジョンモデル（既定 `glm-ocr:q8_0`）で OCR し、読み順どおりの
  `<本>.ocr.md` に保存します。同一フレーズの繰り返しに崩壊した応答は自動検知
  して別モデルで再試行し、それでも失敗したページは `OCR_FAILED` マーク付きで
  残します。キャンセルしても完了済みページは保存されます。前回の結果がある
  場合は**続きから**(完了済みを再利用し、失敗・未処理のみ再実行)を選べ、
  全ページ完了時には抽出テキストを本文とするリフロー型の **`<本>.ocr.epub`**
  (1ページ=1章、縦書き本は縦書き・右綴じ)も生成されます。生成された EPUB
  では検索・翻訳・要約・読み上げなどの機能がそのまま使えます。
...
| `<本>.ocr.md` | 画像ページから抽出した OCR テキスト（翻訳 > 画像ページからテキスト抽出 (OCR)…） |
| `<本>.ocr.epub` | OCR 完了時に生成されるリフロー型テキスト EPUB |
