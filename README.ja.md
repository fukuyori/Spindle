...
- **ローカル AI 翻訳** — [Ollama](https://ollama.com) 連携。原文 / 対訳併記 /
  訳文の 3 モード、モデル・翻訳先言語・エンドポイントを設定可能。
- **ドラッグ＆ドロップ**とコマンドライン引数でのファイルオープン。
...
- **翻訳**：**🌐** をクリックし、モード（原文 / 対訳併記 / 訳文）を選び、モデルと
  翻訳先言語を設定して「再翻訳」を押します。選択したモデルがインストール済みの
  Ollama が起動している必要があります（既定モデルは `qwen2.5`）。
...
各実行で成果物がアップロードされます。`v0.1.0` のようなタグを push すると、
