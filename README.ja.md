...
- **ハイライトとノート** — テキストを選択して 6 色から着色。マークは本文中に
  ...
  保持するため、原文・対訳・訳文のどのビューでも正しい位置を保ちます（引いた側は
  文字単位、反対側はブロック単位の着色で表示）。保存先は EPUB と同じフォルダ。
...
- **ローカル AI 翻訳** — [Ollama](https://ollama.com) 連携。章全体の 3 モード
  ...
  言語・エンドポイントを設定可能。翻訳結果は**本・言語ごとに** EPUB の隣に
  キャッシュするので再読は即時。
...
`Changeling.epub.glossary.json`）。指定した用語の訳し方を、翻訳先言語ごとに固定します:
...
```json
{
  ...
  "langs": {
    "ja": [
      { "source": "Changeling", "target": "チェンジリング", "note": "人名" },
...
| フィールド | 必須 | 意味 |
|------------|------|------|
| `version` | 必須 | 書式バージョン（現在は `1`） |
| `langs` | 必須 | 翻訳先言語コード（`ja` / `en` …＝翻訳ターゲットと同じコード）をキーにしたオブジェクト |
| `langs.<コード>[]` | — | その言語の用語エントリの配列 |
| `source` | 必須 | 原文中に現れる語 |
| `target` | 必須 | 翻訳で使う訳語 |
| `note` | 任意 | モデルへ渡す短い補足（例 `"人名"`） |

補足:
- **現在の翻訳先言語**のエントリだけが翻訳プロンプトに追加されます。言語ごとに別の
  リストを持ち、翻訳先言語を切り替えるとその言語のセクションが読み込まれます。
- `source` または `target` が空のエントリは無視されます。
